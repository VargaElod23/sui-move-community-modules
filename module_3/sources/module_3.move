module module_3::hero;
    ...
    public struct ListHero has key, store {
        ...
        seller: address,
        // 5. The timestamp of the ListHero
        timestamp: u64,
    }
    ...
    public entry fun create_hero(name: String, image_url: String, power: u64, clock: &clock::Clock, ctx: &mut TxContext) {
        let hero = Hero {
            id: object::new(ctx),
            name,
            image_url,
    ...
    public entry fun list_hero(nft: Hero, price: u64, clock: &clock::Clock, ctx: &mut TxContext) {
    ...
    public entry fun buy_hero(list_hero: ListHero, coin: Coin<SUI>, clock: &clock::Clock, ctx: &mut TxContext) {
        // TODO: Deconstruct the ListHero object
        let ListHero { id, nft, price, seller, timestamp } = list_hero;
        // TODO: Assert the price of the Hero is equal to the coin amount
        assert!(coin::value(&coin) == price, EInvalidPrice);
        ...
        let bought_id = object::uid_to_inner(&id);
